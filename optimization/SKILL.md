...
Unix 设计哲学与 Xuan Master 认知模型存在深层同构——两者本质上都是关于 **如何应对复杂性** 的方法论。完整映射参考 `00-entry-enabled/references/unix-design-philosophy-mapping.md`。
...
`00-entry-enabled/` 是认知内核之上的执行力外壳。技能家族现在包括三层：

```
应用层    → 具体问题→具体产出
使能层    → 5-phase协议 (诊断→执行→合成→产出→反馈)
内核层    → 27个认知模型
```

优化内核层模型时需更新使能层：
- 新增模型 → 更新 `00-entry-enabled/SKILL.md` 的映射表
