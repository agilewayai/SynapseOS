...
Unix 设计哲学与 Xuan Master 认知模型存在深层同构——两者本质上都是关于 **如何应对复杂性** 的方法论。完整映射参考 `archon/enabled/references/unix-design-philosophy-mapping.md`。
...
`archon/enabled/` 是认知内核之上的执行力外壳，现归属于 `Archon`。技能家族现在包括三层：
...
优化内核层模型时需更新使能层：
- 新增模型 → 更新 `archon/enabled/SKILL.md` 的映射表
